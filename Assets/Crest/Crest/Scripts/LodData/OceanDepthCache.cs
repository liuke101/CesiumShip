...
#pragma warning restore 414
    ...
    public partial class OceanDepthCache : IValidated
    {
        ...
        public bool Validate(OceanRenderer ocean, ValidatedHelper.ShowMessage showMessage)
        {
            ...
            isBroken = int.Parse(System.Text.RegularExpressions.Regex.Match(Application.unityVersion.Split('.')[^1], @"^\d+").Value) < 23;
#elif UNITY_2021_3_OR_NEWER
            isBroken = int.Parse(System.Text.RegularExpressions.Regex.Match(Application.unityVersion.Split('.')[^1], @"^\d+").Value) < 45;
#endif
            if (isBroken)
            {
                        ...
                        foreach (var renderer in urpRenderers)
                        {
                            ...
                            if (urpRenderer.depthPrimingMode != DepthPrimingMode.Disabled)
                            {
                                ...
                                (
                                    ...
                                    $"Unity fixed this in 2021.3.45f1, 2022.3.23f1 and Unity 6.",
